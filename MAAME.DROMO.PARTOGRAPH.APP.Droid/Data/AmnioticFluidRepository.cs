    ...
    public class AmnioticFluidRepository : BasePartographRepository<AmnioticFluid>
    {
        ...
        protected override AmnioticFluid MapFromReader(SqliteDataReader reader)
        {
            return new AmnioticFluid
            {
                ...
                Color = reader.IsDBNull(5) ? string.Empty : reader.GetString(5),
        ...
        INSERT INTO Tbl_AmnioticFluid (ID, partographID, time, handler, notes, color, createdtime, updatedtime, deletedtime, deviceid, origindeviceid, syncstatus, version, serverversion, deleted, datahash)
        VALUES (@id, @partographId, @time, @handler, @notes, @color, @createdtime, @updatedtime, @deletedtime, @deviceid, @origindeviceid, @syncstatus, @version, @serverversion, @deleted, @datahash);";
